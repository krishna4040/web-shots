    ...
    return (
        <div className="flex items-center justify-center w-[99%] mx-auto lg:justify-between mb-5">
            <div className="flex-wrap items-center justify-center hidden gap-5 lg:flex lg:justify-start">
                ...
                <button onClick={clickHandler} className={`px-4 py-2 border rounded-md bg-slate-200 hover:scale-110 duration-200 ${clciked == 'Sports' ? 'bg-slate-900 text-white' : ''}`}>Sports</button>
                <button onClick={clickHandler} className={`px-4 py-2 border rounded-md bg-slate-200 hover:scale-110 duration-200 ${clciked == 'EnterTainment' ? 'bg-slate-900 text-white' : ''}`}>EnterTainment</button>
                <button onClick={clickHandler} className={`px-4 py-2 border rounded-md bg-slate-200 hover:scale-110 duration-200 ${clciked == 'Lifestyle' ? 'bg-slate-900 text-white' : ''}`}>Lifestyle</button>
                <button onClick={clickHandler} className={`px-4 py-2 border rounded-md bg-slate-200 hover:scale-110 duration-200 ${clciked == 'Science and Tech' ? 'bg-slate-900 text-white' : ''}`}>Science and Tech</button>
                <button onClick={clickHandler} className={`px-4 py-2 border rounded-md bg-slate-200 hover:scale-110 duration-200 ${clciked == 'Travel' ? 'bg-slate-900 text-white' : ''}`}>Travel</button>
            ...
            <div className="flex items-center justify-center gap-2">
                ...
                <label htmlFor="search" className="dark:text-white" onClick={() => {setTopic(inp.search);console.log('called')}}><BiSearchAlt/></label>
            </div>
        </div>
    )
}